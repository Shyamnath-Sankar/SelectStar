...
export async function POST(req: NextRequest) {
  ...
  // Use domain-specific starter questions when a template matches, otherwise
  // fall back to the generic heuristic starter list.
  const { questions: suggestedQuestions, domain } = getStarterQuestions(schema, suggestStarterQuestions);
  ...
  const response: ConnectResponse = {
    ...
    canWrite,
    suggestedQuestions,
    // Surface the detected domain so the client can show a badge + use the
    // template's default report plan when the user clicks "Generate Report".
