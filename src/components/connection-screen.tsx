  ...
  async function reopenSession(s: RecentSession) {
    ...
    try {
      ...
      loadSession({
        ...
        suggestedQuestions: data.suggestedQuestions || [],
        messages: (data.messages || []).map((m: { role: string; content: string; id?: string; inlineArtifacts?: import("@/lib/types").CanvasObject[] }, i: number) => ({
          id: m.id || `msg-${i}`,
          role: m.role as "user" | "assistant" | "system",
          content: m.content,
          // Restore HITL artifacts (pending_write, report_plan) so they
          // re-render inline in the chat bubble after a session reload.
          inlineArtifacts: m.inlineArtifacts,
        })),
        canvas: data.canvas || [],
      });
