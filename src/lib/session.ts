...
export async function loadSessionState(sessionId: string): Promise<SessionState | null> {
  ...
  // Build messages, restoring any persisted inline HITL artifacts
  // (pending_write, report_plan) from the message meta so they re-render
  // inside the chat bubble on session reload.
  const messages: AgentMessage[] = session.messages.map((m) => {
    const meta = m.meta ? (safeParse(m.meta) ?? {}) : {};
    return {
      role: m.role as AgentMessage["role"],
      content: m.content,
      inlineArtifacts: Array.isArray((meta as { inlineArtifacts?: unknown }).inlineArtifacts)
        ? (meta as { inlineArtifacts: CanvasObject[] }).inlineArtifacts
        : undefined,
    };
  });
...
export async function loadCanvasHistory(sessionId: string): Promise<CanvasObject[]> {
  ...
  // HITL artifacts (pending_write, report_plan) live inline in chat messages,
  // not on the canvas pane — filter them out so the canvas stays focused on
  // read-only analytical artifacts (tables, charts, EDA, models, reports).
  return rows
    .map((r) => JSON.parse(r.data) as CanvasObject)
    .filter((o) => o.type !== "pending_write" && o.type !== "report_plan");
}
