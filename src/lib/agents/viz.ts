...
{
  "title": "short chart title WITH a one-line annotation of the key finding (e.g. 'Revenue by Month — +34% growth from Jan to Dec')",
  ...
  "caption": "one short sentence explaining what the chart shows in plain English — no jargon"
}
...
2. **PLAIN-ENGLISH TITLES (CRITICAL — non-expert users):**
   - The title must include a one-line annotation of the key finding — e.g. "Top Products by Revenue — top 3 drive 60% of sales" or "Revenue by Month — sales up 34% over the period".
   - Use plain English. Avoid jargon like "distribution", "variance", "aggregation".
   - Numbers in the title should be the most striking finding, computed from the sample data.
3. Always set a title, an x and/or y encoding, and "tooltip": true.
4. Reference column names EXACTLY as given — they are case-sensitive.
5. For large value ranges, add "scale": { "type": "sqrt" } where it helps.
6. Never include a "data" key — the orchestrator injects the dataset inline as "values" for you.
7. Keep the spec minimal and valid. No transforms unless genuinely needed.
8. For HISTOGRAMS (distribution of a single numeric column): the encoding MUST be:
...
9. The caption should explain what the chart shows in one short sentence a non-expert can understand. Avoid the words "visualization", "encoding", "spec".`;
...
Decide the best chart type and return ONLY the JSON spec. Remember: do NOT include a "data" key.`,
  ...
  // Auto-augment the title with a plain-English key finding if the LLM
  // didn't include one. This is a safety net — the system prompt asks for
  // annotated titles, but we can't guarantee the LLM followed it.
  if (!title.includes("—") && !title.includes(":")) {
    const annotation = computeAnnotation(frame, spec);
    if (annotation) {
      title = `${title} — ${annotation}`;
    }
  }
  // Same for caption — if empty, generate a plain-English one.
  if (!caption) {
    caption = computePlainCaption(frame, spec);
  }
...
/**
 * Compute a plain-English key-finding annotation from the data + spec.
 * Looks at the encoding channels and computes a striking statistic
 * (top category's share, growth rate, max value, etc.).
 */
function computeAnnotation(
  frame: { columns: { name: string }[]; rows: Record<string, unknown>[] },
  spec: Record<string, unknown>
): string | null {
  const encoding = spec.encoding as Record<string, { field?: string; aggregate?: string; type?: string }>;
  if (!encoding) return null;

  // Find the quantitative channel (the "value") and the dimensional channel.
  const channels = Object.entries(encoding);
  let field: string | null = null;
  let dimField: string | null = null;
  for (const [, c] of channels) {
    if (c?.type === "quantitative" && c.field) field = c.field;
    if ((c?.type === "nominal" || c?.type === "temporal" || c?.type === "ordinal") && c.field) dimField = c.field;
  }

  // For histograms (binned x), we can't easily compute a striking stat — skip.
  if (!dimField && field) {
    // Single quantitative column → maybe a histogram. Report the range.
    const vals = frame.rows.map((r) => Number(r[field!])).filter((n) => !Number.isNaN(n));
  ...
  if (dimField && field) {
    // Aggregate by dimension, find the top contributor.
    const totals = new Map<string, number>();
    let grand = 0;
    for (const r of frame.rows) {
      const k = String(r[dimField!] ?? "(null)");
    ...
    // If temporal, compute growth rate from first to last.
    if (encoding.x?.type === "temporal" || encoding.y?.type === "temporal") {
      const sorted = [...frame.rows]
        .filter((r) => !Number.isNaN(Number(r[field!])))
        .sort((a, b) => String(a[dimField!]).localeCompare(String(b[dimField!])));
      if (sorted.length >= 2) {
        const first = Number(sorted[0][field!]);
        const last = Number(sorted[sorted.length - 1][field!]);
        if (first !== 0) {
          const growth = ((last - first) / Math.abs(first)) * 100;
          if (Math.abs(growth) > 5) {
            return `${growth > 0 ? "+" : ""}${growth.toFixed(0)}% ${growth > 0 ? "growth" : "decline"} over the period`;
          }
        }
      }
    }
    // Otherwise report concentration.
    return `top "${ranked[0][0]}" drives ${topShare.toFixed(0)}% of ${field}`;
  }
  return null;
}

/** Generate a plain-English caption for a chart, in case the LLM didn't. */
function computePlainCaption(
  frame: { columns: { name: string }[]; rows: Record<string, unknown>[] },
  spec: Record<string, unknown>
): string {
  const encoding = spec.encoding as Record<string, { field?: string; type?: string }>;
  if (!encoding) return "Chart of the query results.";
  const fields = Object.values(encoding).map((c) => c?.field).filter(Boolean) as string[];
  if (!fields.length) return "Chart of the query results.";
  const markType = typeof spec.mark === "object" && spec.mark ? (spec.mark as { type?: string }).type : "chart";
  return `${markType} chart showing ${fields.join(" by ")} — ${frame.rows.length.toLocaleString()} rows.`;
}

function fmt(x: number): string {
  if (Math.abs(x) >= 1_000_000) return `${(x / 1_000_000).toFixed(1)}M`;
  if (Math.abs(x) >= 1_000) return `${(x / 1_000).toFixed(1)}k`;
  if (Math.abs(x) >= 1) return x.toFixed(0);
  return x.toFixed(2);
}
