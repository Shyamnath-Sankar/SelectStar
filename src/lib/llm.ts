 ...
 *
 * FALLBACK: when LLM_API_KEY is unset, the wrapper transparently uses the
 * `z-ai-web-dev-sdk` (a built-in OpenAI-compatible client that ships with
 * the project). This means the app works out-of-the-box with no API key —
...
/** True when an explicit API key + base URL is configured. False → use z-ai fallback. */
const USE_OPENAI_COMPATIBLE = Boolean(API_KEY);
...
/**
 * Lazily create a z-ai-web-dev-sdk client. The SDK is async-initialized, so
 * we cache the Promise and await it on every call. Returns an object shaped
 * like the OpenAI SDK's `chat.completions` API.
 */
async function zaiClient(): Promise<{
  completions: {
  ...
  if (!_zaiPromise) {
    // Dynamic import so the SDK doesn't get bundled if a real API key is set.
    _zaiPromise = (async () => {
...
export async function complete(
  ...
  // Fallback path: no API key configured → use z-ai-web-dev-sdk.
  if (!USE_OPENAI_COMPATIBLE) {
    try {
      ...
      const completion = await zai.completions.create(payload);
...
export async function completeStream(
  ...
  // Fallback path: no API key → z-ai SDK (non-streaming + pseudo-chunked typing).
  if (!USE_OPENAI_COMPATIBLE) {
