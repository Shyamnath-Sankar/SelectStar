 ...
 *
 * Creates a sample e-commerce SQLite database at `db/demo.db` with realistic
 * data so the platform is fully explorable out of the box. The user can also
 * paste their own connection string to connect to a real database.
 *
 * Run with: `bun run scripts/seed-demo.ts`
 */
// Uses bun:sqlite when run under `bun`, falls back to better-sqlite3 under node.
import { Database } from "bun:sqlite";
import { randomInt } from "crypto";
import { mkdirSync } from "fs";
import { dirname } from "path";

const DB_PATH = "db/demo.db";
...
// ---- Products -----------------------------------------------------------
const productTemplates: Record<string, [string, number, number][]> = {
  Audio: [
    ["Wireless Headphones", 199.99, 88],
    ["Bluetooth Speaker", 79.99, 34],
    ["Studio Monitor", 349.0, 180],
    ["USB Microphone", 129.0, 52],
  ],
  Wearables: [
    ["Fitness Tracker", 99.0, 38],
    ["Smartwatch Pro", 299.0, 140],
    ["GPS Watch", 449.0, 210],
  ],
  Kitchen: [
    ["Espresso Machine", 599.0, 280],
    ["Air Fryer", 129.99, 58],
    ["Blender", 89.0, 41],
    ["Knife Set", 149.0, 62],
  ],
  Furniture: [
    ["Ergonomic Chair", 449.0, 190],
    ["Standing Desk", 599.0, 260],
    ["Bookshelf", 179.0, 80],
  ],
  Stationery: [
    ["Notebook Pack", 24.99, 9],
    ["Premium Pen", 49.0, 18],
    ["Desk Organizer", 39.0, 14],
  ],
  Books: [
    ["Data Engineering 101", 45.0, 16],
    ["SQL Mastery", 38.0, 12],
    ["ML Foundations", 52.0, 20],
  ],
};
const insertProduct = db.prepare(
  "INSERT INTO products (id, name, category_id, price, cost, stock, discontinued) VALUES (?, ?, ?, ?, ?, ?, ?)"
);
const products: { id: number; name: string; price: number; cost: number; catId: number }[] = [];
let pid = 1;
...
const insertCust = db.prepare(
  "INSERT INTO customers (id, name, email, region_id, signup_date, is_vip) VALUES (?, ?, ?, ?, ?, ?)"
);
const customerIds: number[] = [];
const N_CUSTOMERS = 240;
...
for (let i = 0; i < N_ORDERS; i++) {
  ...
  insertOrder.run(orderId, custId, orderDate, status, Math.round(total * 100) / 100);
  for (const { p, qty } of items) {
    insertItem.run(itemId, orderId, p.id, qty, p.price);
    itemId++;
  }
  orderId++;
}
...
const stats = {
  regions: (db.prepare("SELECT COUNT(*) c FROM regions").get() as { c: number }).c,
